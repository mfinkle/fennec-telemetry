/*
Mostly copied from http://bl.ocks.org/kerryrodden/7090426
...
function getColorForStep(step) {
	var keys = d3.keys(colors);
	for (var i=0; i<keys.length; i++) {
		if (step.indexOf(keys[i]) == 0)
...
d3.text("data/activity.csv", function(text) {
...
function buildHierarchy(csv) {
	...
	for (var i = 0; i < csv.length; i++) {
		if (csv[i][1] != "beta") continue;
		var sequence = csv[i][2];
		var size = +csv[i][3];
