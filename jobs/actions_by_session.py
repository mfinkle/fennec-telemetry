    ...
    if len(ui) > 0:
      ...
      for event in ui:
        if event["type"] == "event" and "action." in event["action"]:
          add_to_actions(actions, event["extras"], event["method"], concat_sessions(event["sessions"]))
...
def add_to_actions(actions, action, method, sessions):
  identifer = action + " | " + method + " | " + sessions
